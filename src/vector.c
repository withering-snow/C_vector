...
static void _vector_realloc(vector *v, size_t new_capacity) {
    ...
    // 进行内存扩充
    char *new_begin = (char *)realloc((char *)v->begin, new_capacity * v->element_size);

    // 确认内存分配成功
    VECTOR_ASSERT(new_begin != NULL, "VECTOR_EXPAND_ERROR");

    char *new_end = new_begin + len * v->element_size;
    char *new_end_of_storage = new_begin + new_capacity * v->element_size;

    v->begin = new_begin;
    v->end = new_end;
    v->end_of_storage = new_end_of_storage;
}
...
void _vector_reserve(vector *v, size_t capacity) {
    // 对预留空间过小的检测
    VECTOR_ASSERT(_vector_size(v) <= capacity, "VECTOR_RESERVE_TOO_SMALL");
